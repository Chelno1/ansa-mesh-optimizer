...
def create_individual(bounds: List[Tuple[float, float]], param_types: List[type]) -> Individual:
    """
    创建随机个体
...
def create_population(size: int, bounds: List[Tuple[float, float]], param_types: List[type]) -> List[Individual]:
    """
    创建随机种群
...
def create_lhs_population(size: int, bounds: List[Tuple[float, float]], param_types: List[type]) -> List[Individual]:
    """
    使用拉丁超立方采样创建种群
    ...
    try:
        ...
        for sample in lhs_samples:
            genes = []
            for i, (s, (low, high), param_type) in enumerate(zip(sample, bounds, param_types)):
                if param_type == int:
