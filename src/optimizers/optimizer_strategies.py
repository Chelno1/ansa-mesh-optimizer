...
class BayesianOptimizerStrategy(OptimizerStrategy):
    ...
    def _format_skopt_result(self, result) -> "OptimizationResult":
        ...
        logger.info(
            f"skopt_result属性: func_vals={hasattr(result, 'func_vals')}, x_iters={hasattr(result, 'x_iters')}, space={hasattr(result, 'space')}"
        )
        ...
        optimization_result = self._format_result(
            ...
            {"skopt_result": result, "convergence_info": convergence_info},
        )

        logger.info(
            f"创建的OptimizationResult有skopt_result: {hasattr(optimization_result, 'skopt_result')}"
        )
        if hasattr(optimization_result, "skopt_result"):
            logger.info(
                f"skopt_result是否为None: {optimization_result.skopt_result is None}"
            )
    ...
    def optimize(self, n_calls: int, **kwargs) -> "OptimizationResult":
        ...
        try:
            ...
            if hasattr(genetic_optimizer, "optimization_history"):
                self.optimization_history = getattr(
                    genetic_optimizer, "optimization_history", []
                )
...
class ParallelGeneticOptimizerStrategy(OptimizerStrategy):
    ...
    def optimize(self, n_calls: int, **kwargs) -> "OptimizationResult":
        """执行并行遗传算法优化"""
        from .parallel_genetic_optimizer import (
            ParallelGeneticConfig,
        ...
        parallel_config = ParallelGeneticConfig(
            ...
            elite_size=max(
                1,
                int(
                    getattr(self.config, "population_size", 50)
                    * getattr(self.config, "elitism_ratio", 0.1)
                ),
            ),
            ...
            max_stagnation_iterations=getattr(
                self.config, "max_stagnation_iterations", 20
            ),
        ...
        logger.info(
            f"开始并行遗传算法优化: 工作进程数={parallel_config.n_workers}, 种群大小={parallel_config.population_size}"
        )
        ...
        if hasattr(parallel_optimizer, "optimization_history"):
            self.optimization_history = getattr(
                parallel_optimizer, "optimization_history", []
            )
        ...
        if hasattr(parallel_optimizer, "performance_monitor"):
            performance_summary = (
                parallel_optimizer.performance_monitor.get_performance_summary()
            )
...
class ParallelOptimizerStrategy(OptimizerStrategy):
    ...
    def optimize(self, n_calls: int, **kwargs) -> "OptimizationResult":
        ...
        return self._format_result(
            best_params, best_value, {"all_results": all_results}
        )
    ...
    def _generate_random_params(self, n_samples: int) -> List[Dict[str, float]]:
        ...
        for _ in range(n_samples):
            params = {}
            for i, (name, bound, param_type) in enumerate(
                zip(param_names, bounds, param_types)
            ):
                if isinstance(bound, tuple) and len(bound) == 2:
                    low, high = bound
                    if param_type is int or param_type == "integer":
...
class OptimizerFactory:
    ...
    def create_optimizer(
        cls, optimizer_type: str, param_space, evaluator, config
    ) -> OptimizerStrategy:
        """
        创建优化器策略实例
        ...
        if optimizer_type not in cls._strategies:
            ...
            raise ValueError(
                f"不支持的优化器类型: {optimizer_type}. 可用类型: {available}"
            )

        # 检查依赖
        if (
            optimizer_type in ["bayesian", "random", "forest"]
            and not SKOPT_MODULES["available"]
        ):
