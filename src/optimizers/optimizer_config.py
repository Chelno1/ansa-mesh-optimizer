...
class OptimizerConfig:
    ...
    def _validate_config(self):
        ...
        if self.acquisition_function not in valid_acquisition_functions:
            raise ValueError(
                f"acquisition_function必须是{valid_acquisition_functions}之一"
            )
    ...
    def get_optimizer_specific_config(self, optimizer_type: str) -> Dict[str, Any]:
        """获取特定优化器的配置"""
        base_config = {"random_state": self.random_state, "verbose": self.verbose}
...
class OptimizationResult:
    ...
    def _setup_skopt_compatibility(self):
        ...
        self.message = (
            ...
            if self.success
            else (self.error_message or "Optimization failed")
        )
    ...
    def from_genetic_result(
        ...
        # 将参数列表转换为字典
        best_params_dict = {
            parameter_names[i]: best_params[i] for i in range(len(parameter_names))
        }
