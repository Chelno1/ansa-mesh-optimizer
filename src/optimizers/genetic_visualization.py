...
class GeneticVisualizer:
    ...
    def __init__(self, best_fitness_history: List[float], diversity_history: List[float], 
                 generation_stats: List[Dict[str, Any]], genetic_config: Optional[Dict] = None):
        """
        初始化可视化器
    ...
    def plot_convergence_analysis(self, save_path: Optional[str] = None) -> None:
        ...
        try:
            ...
            # 多样性与适应度关系
            if self.diversity_history and len(self.diversity_history) == len(self.best_fitness_history):
                axes[1, 0].scatter(self.diversity_history, self.best_fitness_history, 
            ...
            if self.generation_stats:
                ...
                axes[1, 1].plot(generations, mean_fitness, 'b-', linewidth=2, label='平均适应度')
                axes[1, 1].fill_between(generations, 
...
def plot_evolution_history(best_fitness_history: List[float],
...
def export_evolution_data(best_fitness_history: List[float],
...
def create_genetic_visualizer(best_fitness_history: List[float],
                            diversity_history: List[float],
                            generation_stats: List[Dict[str, Any]],
                            genetic_config: Optional[Dict] = None) -> GeneticVisualizer:
    """
    创建遗传算法可视化器的工厂函数
