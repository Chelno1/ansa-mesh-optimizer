...
class GeneticOptimizer:
    ...
    def __init__(
        ...
        logger.info(
            f"遗传算法优化器初始化完成 - 种群大小: {self.genetic_config.population_size}"
        )
    ...
    def optimize(self, n_calls: int, **kwargs) -> OptimizationResult:
        ...
        # 计算动态参数
        population_size = min(
            self.genetic_config.population_size, max(10, n_calls // 5)
        )
        max_generations = min(
            self.genetic_config.max_generations, n_calls // population_size
        )

        logger.info(
            f"开始遗传算法优化: 种群={population_size}, 最大代数={max_generations}"
        )

        try:
            # 执行进化循环
            total_generations, total_evaluations, execution_time = (
                self.evolution_loop.run_evolution(
                    n_calls=n_calls,
    ...
    def export_evolution_data(self, filename: str) -> None:
        ...
        try:
            ...
            best_individual_info = {
                ...
                "params": (
                    best_individual.to_params(self.param_names)
                    if best_individual
                    else None
                ),
...
def create_genetic_optimizer(
    param_space, evaluator, config: Optional[Any] = None, **genetic_kwargs
) -> GeneticOptimizer:
    """
    创建遗传算法优化器的工厂函数
