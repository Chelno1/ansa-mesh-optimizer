...
class EvolutionEngine:
    ...
    def initialize_population(self, population_size: int) -> List[Individual]:
        ...
        try:
            ...
            lhs_population = create_lhs_population(
                lhs_size, self.bounds, self.param_types
            )
    ...
    def evolve_population(
        ...
        while len(new_population) < len(population):
            ...
            child1, child2 = parent1.crossover(
                parent2, self.genetic_config.crossover_rate
            )
    ...
    def roulette_wheel_selection(self, population: List[Individual]) -> Individual:
        ...
        # 计算适应度值（转换为最大化问题）
        fitness_values = [
            1.0 / (1.0 + ind.fitness) if ind.fitness is not None else 0.0
    ...
    def update_best_individual(
        self, population: List[Individual], best_individual: Optional[Individual]
    ) -> Individual:
        """更新最佳个体"""
        ...
        if best_individual is None or (
            ...
            and (
                best_individual.fitness is None
    ...
    def check_convergence(self) -> bool:
        ...
        # 检查最近几代的改进
        recent_best = self.best_fitness_history[
        ...
        if len(self.best_fitness_history) >= 2:
            if (
                abs(self.best_fitness_history[-1] - self.best_fitness_history[-2])
                < self.genetic_config.convergence_threshold
            ):
        ...
        # 收敛条件
        return (
            self.convergence_counter >= self.genetic_config.convergence_patience
            or self.stagnation_counter >= self.genetic_config.convergence_patience * 2
        )
    ...
    def calculate_improvement_ratio(self) -> float:
        ...
        if len(self.best_fitness_history) > 1:
            ...
            return (
                (initial_fitness - final_fitness) / initial_fitness
...
def uniform_crossover(
    parent1: Individual, parent2: Individual, crossover_rate: float = 0.8
...
def gaussian_mutation(
    individual: Individual, mutation_rate: float = 0.1, mutation_strength: float = 0.1
) -> None:
    """
    高斯变异操作
