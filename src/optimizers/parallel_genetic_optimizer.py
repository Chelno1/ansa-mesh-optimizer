...
class ParallelGeneticConfig(GeneticConfig):
    ...
    def _validate_parallel_config(self):
        ...
        if self.n_workers > multiprocessing.cpu_count():
            logger.warning(f"工作进程数({self.n_workers})超过CPU核心数({multiprocessing.cpu_count()})")
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                'avg_evaluation_time': np.mean(self.metrics['evaluation_times']) if self.metrics['evaluation_times'] else 0,
                'avg_evolution_time': np.mean(self.metrics['evolution_times']) if self.metrics['evolution_times'] else 0,
                'avg_diversity_time': np.mean(self.metrics['diversity_times']) if self.metrics['diversity_times'] else 0,
...
class ParallelEvaluationEngine:
    ...
    def __init__(self, evaluator, config: ParallelGeneticConfig, param_names: List[str]):
    ...
    def _initialize_parallel_resources(self):
        ...
        if self.config.use_multiprocessing and self.config.parallel_evaluation:
            try:
                ...
                logger.info(f"并行评估引擎初始化完成 - 工作进程数: {self.config.n_workers}")
            except Exception as e:
    ...
    def _evaluate_population_multiprocess(self, population: List[Individual]) -> int:
        ...
        for batch_start in range(0, len(population), batch_size):
            ...
            for individual in batch:
                if individual.fitness is None:
                    ...
                    task_args = (self.evaluator, params, individual_data, self.config.timeout_seconds)
            ...
            try:
                ...
                if self.process_pool is not None:
                    for task_args in tasks:
                        future = self.process_pool.submit(evaluate_individual_worker, task_args)
                ...
                for i, future in enumerate(futures):
                    try:
                        fitness, eval_time = future.result(timeout=self.config.timeout_seconds)
                        individuals_to_evaluate[i].fitness = fitness
                        
                        # 更新缓存
                        if self.evaluation_cache is not None and fitness != float('inf'):
                            params = individuals_to_evaluate[i].to_params(self.param_names)
                            cache_key = self._get_cache_key(params)
                        ...
                        if eval_time is not None:
                            self.performance_monitor.metrics['evaluation_times'].append(eval_time)
        ...
        if evaluation_count > 0:
            total_time = sum(self.performance_monitor.metrics.get('batch_evaluation_times', [0]))
            if total_time > 0:
...
class ParallelDiversityEngine:
            ...
            for i in range(0, n, chunk_size):
                ...
                if self.thread_pool is not None:
                    future = self.thread_pool.submit(
                        self._calculate_chunk_distances,
                        chunk, remaining
                    )
                    futures.append(future)
                else:
                    # 回退到串行计算
                    chunk_distance, chunk_pairs = self._calculate_chunk_distances(chunk, remaining)
    ...
    def _calculate_chunk_distances(self, chunk: List[Individual], 
                                 remaining: List[Individual]) -> Tuple[float, int]:
        """计算块内和块间距离"""
        total_distance = 0.0
        count = 0
...
class ParallelGeneticOptimizer(GeneticOptimizer):
    ...
    def optimize(self, n_calls: int, **kwargs) -> OptimizationResult:
        """并行优化主流程"""
        logger.info(f"开始并行遗传算法优化: 评估次数={n_calls}, 工作进程={self.parallel_config.n_workers}")
    ...
    def _optimize_parallel(self, n_calls: int, **kwargs) -> OptimizationResult:
        """标准并行优化"""
        # 调整参数
        population_size = min(self.parallel_config.population_size, max(10, n_calls // 5))
        max_generations = min(self.parallel_config.max_generations, n_calls // population_size)
        
        logger.info(f"并行优化参数: 种群大小={population_size}, 最大代数={max_generations}")
        ...
        try:
            ...
            with self.performance_monitor.measure_time('initial_evaluation'):
                total_evaluations = self.evaluation_engine.evaluate_population_parallel(population)
            ...
            for generation in range(max_generations):
                ...
                with self.performance_monitor.measure_time('evaluation'):
                    new_evaluations = self.evaluation_engine.evaluate_population_parallel(new_population)
            ...
            logger.info(f"并行优化完成: 最佳适应度={result.best_score:.6f}, "
    ...
    def _evolve_population_parallel(self, population: List[Individual], 
                                  generation: int, max_generations: int) -> List[Individual]:
        """并行进化种群"""
        # 排序种群
        population.sort()
    ...
    def _generate_offspring_parallel(self, population: List[Individual], 
                                   offspring_needed: int, generation: int, 
                                   max_generations: int, mutation_rate: float) -> List[Individual]:
        """并行生成后代"""
        offspring = []
        ...
        with ThreadPoolExecutor(max_workers=self.parallel_config.n_workers) as executor:
            ...
            for batch_start in range(0, offspring_needed, batch_size):
                ...
                future = executor.submit(
                    ...
                    population, batch_size_actual, generation, max_generations, mutation_rate
                )
                futures.append(future)
    ...
    def _generate_offspring_serial(self, population: List[Individual], 
                                 offspring_needed: int, generation: int, 
                                 max_generations: int, mutation_rate: float) -> List[Individual]:
        """串行生成后代"""
    ...
    def _generate_offspring_batch(self, population: List[Individual],
                                batch_size: int, generation: int,
                                max_generations: int, mutation_rate: float) -> List[Individual]:
        """生成一批后代"""
        offspring = []
        ...
        for _ in range((batch_size + 1) // 2):  # 确保生成足够的后代
            ...
            # 交叉
            child1, child2 = parent1.crossover(parent2, self.parallel_config.crossover_rate)
