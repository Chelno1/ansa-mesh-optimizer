...
def validate_ansa_environment(
    ansa_executable: str, timeout: int = 10
...
def check_input_files(
    input_model_path: str, batch_script_path: str
) -> Tuple[bool, str]:
    """
    检查输入文件是否存在
...
def run_ansa_batch(
    ansa_command: list, temp_dir: str, timeout: int
    ...
    # 执行命令
    result = subprocess.run(
        ansa_command, capture_output=True, text=True, timeout=timeout, cwd=temp_dir
    )

    return result


def handle_ansa_returncode(
    ...
    if result.returncode == 0:
        ...
        error_msg = (
            f"Ansa执行失败，返回代码: {result.returncode}, 错误输出: {result.stderr}"
        )
        logger.error(error_msg)
        return False, error_msg
...
class AnsaEnvironmentValidator:
    ...
    def validate(self) -> bool:
        ...
        is_valid, error_msg = check_input_files(
            str(input_model_path), str(batch_script_path)
        )
