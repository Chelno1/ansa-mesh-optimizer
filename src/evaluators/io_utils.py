...
def copy_mpar_files_to_temp_dir(
    temp_dir: str, criterion_dir: Path, mpar_file_pattern: str
) -> str:
    """
...
def process_parameter_files_in_temp_dir(
    temp_dir: str, params: Dict[str, float], parameter_replacer
) -> None:
    """
    在临时文件夹中处理参数文件替换
    ...
    try:
        ...
        # 使用参数替换管理器处理参数
        updated_file_path = parameter_replacer.process_parameter_replacements(
            original_mpar_path, params
        )
        ...
        if updated_file_path != original_mpar_path:
            ...
            if (
                Path(updated_file_path).exists()
                and Path(updated_file_path) not in temp_files_created
            ):
                try:
                    ...
                    logger.warning(
                        f"清理最终更新文件失败 {updated_file_path}: {cleanup_error}"
                    )

            logger.info(
                f"已将更新内容复制回临时目录文件: {mpar_file}，并清理了 {cleaned_count} 个临时文件"
            )
