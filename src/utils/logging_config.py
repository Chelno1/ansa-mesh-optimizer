...
class LoggingConfig:
    ...
    def _create_formatter(self, format_name: str) -> logging.Formatter:
        ...
        format_string = self.config['format'].get(format_name, self.config['format']['simple'])
    ...
    def update_level(self, level: Union[str, int], module: Optional[str] = None) -> None:
        """
        更新日志级别
    ...
    def add_file_logging(self, filename: str, level: str = 'DEBUG', 
                        format_name: str = 'detailed') -> None:
        """
        添加文件日志
...
def setup_logging(level: Union[str, int] = 'INFO',
                 ...
                 verbose: bool = False,
                 quiet: bool = False,
                 config: Optional[Dict[str, Any]] = None) -> LoggingConfig:
    """
    设置日志配置（简化接口）
...
def create_logger(name: str, level: Optional[Union[str, int]] = None) -> logging.Logger:
    ...
    if level is not None:
        logger.setLevel(LoggingConfig().LEVEL_MAP.get(str(level).upper(), logging.INFO) 
...
def setup_batch_logging(log_level: int = logging.INFO, log_dir: Optional[Path] = None) -> None:
    """
    批处理兼容的日志设置函数
