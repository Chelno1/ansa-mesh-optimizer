...
def setup_numpy_print_options():
    ...
    try:
        np.set_printoptions(
            precision=6, suppress=True, threshold=10, edgeitems=3, linewidth=120
        )
    except Exception as e:
...
def performance_monitor(
    operation_name: str, log_memory: bool = True, log_level: int = logging.INFO
):
    """
    ...
    try:
        ...
        logger.log(
            log_level,
            f"{operation_name} 完成，耗时: {format_execution_time(execution_time)}",
        )
        ...
        if log_memory and start_memory and "error" not in start_memory:
            ...
            if "error" not in end_memory:
                ...
                if isinstance(start_rss, (int, float)) and isinstance(
                    end_rss, (int, float)
                ):
...
def create_progress_callback(
    total_iterations: int, verbose: bool = True, update_interval: int = 1
) -> Callable:
    """
    创建进度回调函数，用于显示操作进度
