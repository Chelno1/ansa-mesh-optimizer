...
class ParameterValidator:
    ...
    def validate_comprehensive(
        self, params: Dict[str, Any], allow_partial: bool = True
                ...
                if name in params:
                    ...
                    try:
                        if hasattr(expected_type, "value"):
                            # 处理枚举类型
                            if expected_type.value == "integer":
                                converted_value: Union[int, float] = int(
                                    round(float(value))
                                )
                            elif expected_type.value == "float":
                        ...
                        if low <= converted_value <= high:
                            ...
                            logger.warning(
                                f"Parameter {name}={converted_value} out of bounds [{low}, {high}]"
                            )
                            # 截断到边界内
                            validated_params[name] = max(
                                low, min(high, converted_value)
                            )
                        ...
                        if (
                            hasattr(expected_type, "value")
                            and expected_type.value == "integer"
                        ):
                    ...
                    default_value = (low + high) / 2
                    if (
                        hasattr(self.param_types[i], "value")
                        and self.param_types[i].value == "integer"
                    ):
                        default_value = int(round(default_value))
                    elif self.param_types[i] is int:
                        default_value = int(round(default_value))
                    validated_params[name] = default_value
