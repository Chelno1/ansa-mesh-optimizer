        ...
        with self._lock:
            ...
            try:
                ...
                if entry and self._is_cache_valid(entry):
                    ...
                    if (
                        not self.use_database
                        and params_hash in self.cache
                        and isinstance(self.cache, OrderedDict)
                    ):
    ...
    def _remove_entry(self, params_hash: str) -> None:
        ...
        try:
            if self.use_database:
                with sqlite3.connect(str(self.db_file)) as conn:
                    conn.execute(
                        "DELETE FROM cache_entries WHERE params_hash = ?",
                        (params_hash,),
                    )
                    conn.commit()
                    ...
                    if isinstance(entry_data, dict):
                        if "params" in entry_data:
                            ...
                            entry = CacheEntry(
                                ...
                                evaluation_time=entry_data.get(
                                    "evaluation_time", time.time()
                                ),
                            )
                    else:
...
class CachedEvaluator:
    ...
    def evaluate_mesh(self, params: Dict[str, float]) -> float:
        ...
        try:
            ...
            # 将结果存入缓存（只缓存有效结果）
            if result != float("inf") and result is not None:
    ...
    def get_evaluation_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "avg_evaluation_time": self._total_evaluation_time
...
if __name__ == "__main__":
    ...
    cache = OptimizationCache(
        cache_file="test_cache.pkl", max_entries=10, use_compression=True
    )
