...
class StatisticalAnalyzer:
        ...
        try:
            ...
            for optimizer in optimizers:
                ...
                if len(successful_runs) > 1:
                    performance_data[optimizer] = [
                        r["best_value"] for r in successful_runs
                    ]
                    time_data[optimizer] = [
                        r.get("execution_time", 0) for r in successful_runs
                    ]
        ...
        with open(stats_report, "w", encoding="utf-8") as f:
            ...
            for optimizer, values in performance_data.items():
                if len(values) >= 3:
                    try:
                        ...
                        f.write(
                            f" ({'正态分布' if p_value > 0.05 else '非正态分布'})\n"
                        )
                    except Exception as e:
                ...
                for j in range(i + 1, len(optimizers_list)):
                    ...
                    if len(values1) >= 3 and len(values2) >= 3:
                        try:
                            ...
                            f.write(
                                f"  结果: {'显著差异' if p_value < 0.05 else '无显著差异'}\n"
                            )
            ...
            if len(performance_data) > 2:
                ...
                try:
                    ...
                    f.write(f"p值: {p_value:.4f}\n")
                    f.write(
                        f"结果: {'组间存在显著差异' if p_value < 0.05 else '组间无显著差异'}\n"
                    )
                except Exception as e:
    ...
    def generate_confidence_intervals(
        self, performance_data: Dict[str, List[float]], confidence_level: float = 0.95
    ) -> Dict[str, Dict[str, float]]:
        """
        生成置信区间
    ...
    def perform_power_analysis(
        self, performance_data: Dict[str, List[float]], alpha: float = 0.05
    ) -> Dict[str, float]:
        """
        执行功效分析
            ...
            for j in range(i + 1, len(optimizers_list)):
                ...
                if len(values1) >= 3 and len(values2) >= 3:
                    try:
                        ...
                        effect_size = (
                            abs(mean1 - mean2) / pooled_std if pooled_std > 0 else 0
                        )
