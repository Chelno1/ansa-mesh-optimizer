...
class ComparisonReporter:
    ...
    def save_all_results(
        ...
        try:
            ...
            self._save_text_report(
                results, comparison_summary, comparison_metadata, optimizers
            )
    ...
    def _save_summary(self, comparison_summary: Any) -> None:
        ...
        if isinstance(comparison_summary, list):
            ...
            try:
                if comparison_summary is not None and hasattr(
                    comparison_summary, "to_dict"
                ):
    ...
    def _save_metadata(
        ...
        metadata = {
            ...
            "configuration": {
                ...
                "total_optimizations": (
                    sum(len(runs) for runs in failed_runs.values())
                    if failed_runs
                    else 0
                ),
    ...
    def _save_text_report(
        ...
        try:
            with open(report_file, "w", encoding="utf-8") as f:
                ...
                f.write(
                    f"  总执行时间: {format_execution_time(comparison_metadata.get('total_execution_time', 0))}\n\n"
                )
                ...
                if isinstance(comparison_summary, list):
                    ...
                    try:
                        summary_data = (
                            comparison_summary.to_dict("records")
                            if hasattr(comparison_summary, "to_dict")
                            else []
                        )
                    except Exception:
                        summary_data = []
                ...
                with open(export_file, "w", newline="", encoding="utf-8") as f:
                    if isinstance(comparison_summary, list) and comparison_summary:
                        writer = csv.DictWriter(
                            f, fieldnames=comparison_summary[0].keys()
                        )
                        writer.writeheader()
    ...
    def generate_summary_statistics(
        self, results: Dict[str, List[Dict[str, Any]]], optimizers: List[str]
    ) -> List[Dict[str, Any]]:
        """
        生成摘要统计信息
