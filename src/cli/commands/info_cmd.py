...
def register_info_command(subparsers):
    ...
    info_parser.add_argument("--check-deps", action="store_true", help="检查依赖库")
    info_parser.add_argument("--check-ansa", action="store_true", help="检查Ansa环境")
    info_parser.add_argument("--performance", action="store_true", help="运行性能测试")
...
def cmd_info(args, modules=None) -> int:
    ...
    if args.check_deps:
        ...
        for display_name, import_name, description, required in dependencies:
            ...
            print(
                f"   {color}{display_name:<20} {description:<15} {status:<10} ({req_text})"
            )
    ...
    if args.check_ansa:
        ...
        try:
            import ansa  # type: ignore # noqa: F401
