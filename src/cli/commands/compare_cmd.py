...
def register_compare_command(subparsers):
    ...
    compare_parser.add_argument(
        "--n-calls", type=int, default=20, help="每个优化器的迭代次数 (默认: 20)"
    )
    compare_parser.add_argument(
        "--n-runs", type=int, default=3, help="每个优化器的运行次数 (默认: 3)"
    )
    compare_parser.add_argument(
        "--no-sensitivity", action="store_true", help="禁用敏感性分析"
    )
    compare_parser.add_argument(
        "--no-report", action="store_true", help="禁用详细报告生成"
    )
    compare_parser.add_argument(
        "--parallel-runs", action="store_true", help="并行运行比较（实验性）"
    )
...
def cmd_compare(args, modules) -> int:
    ...
    try:
        ...
        for optimizer in args.optimizers:
            if (
                optimizer in ["bayesian", "random", "forest"]
        ...
        if unavailable_optimizers:
            print(
                f"⚠️  以下优化器不可用（需要 scikit-optimize）: {', '.join(unavailable_optimizers)}"
            )
            available_optimizers = [
                opt for opt in args.optimizers if opt not in unavailable_optimizers
            ]
        ...
        if "summary" in results and results["summary"]:
            ...
            for optimizer_data in summary_data:
                ...
                print(
                    f"   {opt_name:12}: {mean_val:.6f} ± {std_val:.6f} ({exec_time:.2f}s)"
                )
