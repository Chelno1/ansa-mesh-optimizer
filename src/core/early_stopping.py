...
class EarlyStopping:
    ...
    def __init__(
        ...
        if self.verbose:
            logger.info(
                f"早停机制初始化: patience={patience}, min_delta={min_delta}, mode={mode}"
            )

    def __call__(
        self, current_value: float, current_params: Optional[Dict] = None
    ) -> bool:
        """
        检查是否应该早停
        ...
        if improved:
            ...
            if self.verbose:
                logger.debug(
                    f"发现改善: {current_value:.6f} (迭代 {current_iteration})"
                )
        else:
            self.wait += 1
            if self.verbose:
        ...
        if self.wait >= self.patience:
            ...
            if self.verbose:
                logger.info(
                    f"早停触发：在第{self.stopped_iteration}次迭代，"
    ...
    def get_best_result(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "improvement_rate": (
                self.improvement_count / self.total_iterations
        ...
        for i in range(1, len(self.improvement_history)):
            ...
            if period_length > 1:  # 只记录长度大于1的停滞期
                stagnation_periods.append(
                    {"start": period_start, "end": period_end, "length": period_length}
                )
    ...
    @with_chinese_font
    def plot_history(
        self, save_path: Optional[str] = None, show_improvements: bool = True
    ):
        """
        绘制优化历史 - 自动应用中文字体
        ...
        try:
            ...
            if show_improvements and self.improvement_history:
                improvement_values = [
                    values[i] for i in self.improvement_history if i < len(values)
                ]
                ax1.scatter(
                    self.improvement_history[: len(improvement_values)],
                    improvement_values,
                    c="green",
                    s=50,
                    alpha=0.7,
...
class AdaptiveEarlyStopping(EarlyStopping):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.verbose:
            logger.info(
                f"自适应早停初始化: 初始耐心={initial_patience}, "
    ...
    def __call__(
        ...
        if self.wait >= self.patience:
            ...
            if self.verbose:
                logger.info(
                    f"自适应早停触发：在第{self.stopped_iteration}次迭代，"
    ...
    def _adapt_patience(self):
        ...
        if new_patience > old_patience:
            ...
            if self.verbose:
                logger.info(
                    f"自适应调整: 耐心值从 {old_patience} 增加到 {new_patience} "
    ...
    def get_adaptation_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "adaptation_efficiency": (
                self.improvement_count / self.adaptation_count
    ...
    def add_value(self, value: float) -> Tuple[bool, Dict[str, Any]]:
        ...
        if len(self.values) >= self.min_samples:
            ...
            if len(self.values) > 1:
                relative_change = abs(self.values[-1] - self.values[-2]) / (
                    abs(self.values[-1]) + 1e-8
                )
...
class MultiCriteriaEarlyStopping:
    ...
    def __init__(self, criteria: List[Dict[str, Any]], combination_mode: str = "any"):
        """
        初始化多标准早停
        ...
        for criterion in criteria:
            ...
            if criterion_type == "standard":
                stopper = EarlyStopping(
                    **{k: v for k, v in criterion.items() if k != "type"}
                )
            elif criterion_type == "adaptive":
                stopper = AdaptiveEarlyStopping(
                    **{k: v for k, v in criterion.items() if k != "type"}
                )
            else:
        ...
        logger.info(
            f"多标准早停初始化: {len(self.early_stoppers)} 个标准, 组合模式={combination_mode}"
        )
    ...
    def get_status(self) -> Dict[str, Any]:
        """获取所有早停器的状态"""
        status = {"combination_mode": self.combination_mode, "stoppers": []}
...
def create_early_stopping(config) -> EarlyStopping:
    ...
    if hasattr(config, "adaptive_early_stopping") and config.adaptive_early_stopping:
        ...
        return EarlyStopping(
            patience=config.patience, min_delta=config.min_delta, verbose=config.verbose
        )


# 工厂函数
def create_early_stopping_with_criteria(
    criteria_list: List[Dict[str, Any]], combination_mode: str = "any"
) -> MultiCriteriaEarlyStopping:
    """
    创建多标准早停机制
...
if __name__ == "__main__":
    ...
    adaptive_stopping = AdaptiveEarlyStopping(
        initial_patience=3, adaptation_threshold=2, verbose=True
    )
    ...
    for i, value in enumerate(test_values_2):
        ...
        print(
            f"迭代 {i}: 值={value:.3f}, 应停止={should_stop}, 当前耐心={adaptive_stopping.patience}"
        )
        if should_stop:
            break
    ...
    for i, value in enumerate(convergence_values):
        ...
        print(
            f"迭代 {i}: 值={value:.4f}, 收敛={converged}, 趋势={convergence_detector.get_trend_description()}"
        )
        if converged:
            print("检测到收敛!")
            break
