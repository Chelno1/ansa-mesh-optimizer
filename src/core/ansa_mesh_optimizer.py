...
try:
    ...
    class ConfigManagerWrapper:
        def __init__(self, unified_manager):
            ...
            self.parameter_space = ParameterSpaceWrapper(
                unified_manager.parameter_space
            )
...
class MeshOptimizer:
    ...
    def __init__(
        ...
        # 创建评估器
        self.base_evaluator = create_mesh_evaluator(
            evaluator_type, config_manager=config_manager.unified_manager
        )
    ...
    def optimize(
        self, optimizer: str = "bayesian", n_calls: Optional[int] = None, **kwargs
    ) -> OptimizationResult:
        """
        执行优化
        ...
        if optimizer.lower() not in available_optimizers:
            raise ValueError(
                f"优化器 {optimizer} 不可用。可用优化器: {available_optimizers}"
            )
    ...
    def _generate_optimization_report(self, result: OptimizationResult) -> str:
        ...
        reporter.generate_optimization_report(
            ...
            early_stopping_info=(
                self.early_stopping.get_best_result() if self.early_stopping else None
            ),
        )
    ...
    **kwargs,
    ...
    if config_manager is None:
        ...
        unified_manager = UnifiedConfigManager(
            config_file=config_file, require_config=True
        )
    ...
    # 执行优化
    result = mesh_optimizer.optimize(optimizer=optimizer, n_calls=n_calls, **kwargs)
