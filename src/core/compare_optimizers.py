...
class OptimizationComparison:
    ...
    def __init__(
        ...
        logger.info(
            f"并行执行: {self.parallel_execution}, 最大工作线程: {self.max_workers}"
        )
        ...
        **kwargs,
        ...
        try:
            ...
            if self.parallel_execution and len(self.optimizers) > 1:
                self._run_parallel_comparison(
                    run_sensitivity_analysis, save_individual_reports, **kwargs
                )
            else:
                self._run_sequential_comparison(
                    run_sensitivity_analysis, save_individual_reports, **kwargs
                )
            ...
            logger.info(
                f"比较完成！总执行时间: {format_execution_time(total_execution_time)}"
            )
    ...
    def _check_optimizers_availability(self) -> List[str]:
        """检查优化器可用性"""
        try:
            from .ansa_mesh_optimizer import check_dependencies
            ...
            for optimizer in self.optimizers:
                if optimizer in ["bayesian", "random", "forest"]:
                    if deps["skopt_available"]:
                        ...
                        logger.warning(
                            f"优化器 {optimizer} 不可用: 需要 scikit-optimize"
                        )
    ...
    def _run_parallel_comparison(
        self, run_sensitivity_analysis: bool, save_individual_reports: bool, **kwargs
    ) -> None:
        """并行执行比较"""
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            future_to_task = {
                executor.submit(
                    self._run_single_optimization, optimizer, run_idx, kwargs
                ): (optimizer, run_idx)
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    ...
                    self.execution_times[optimizer].append(
                    ...
                    logger.info(
                        f"并行执行进度: {progress:.1f}% ({completed}/{len(tasks)})"
                    )
    ...
    def _run_single_optimization(
        self, optimizer: str, run_idx: int, extra_kwargs: Dict[str, Any]
    ) -> Dict[str, Any]:
        """运行单次优化"""
        # 为每次运行设置不同的随机种子
        ...
        run_config.random_state = (
            original_seed + run_idx * 1000 + hash(optimizer) % 1000
        )

        try:
            with performance_monitor(f"{optimizer} 优化 (运行 {run_idx + 1})"):
                # 创建配置管理器包装器
                from .ansa_mesh_optimizer import ConfigManagerWrapper
                ...
                result = mesh_optimizer.optimize(
                    optimizer=optimizer, n_calls=self.n_calls, **extra_kwargs
                )
    ...
    def _generate_comparison_summary(self) -> None:
        ...
        for optimizer in self.optimizers:
            ...
            summary_entry = {
                ...
                "median_best_value": (
                    stats.get("median", 0) if isinstance(stats, dict) else 0
                ),
                ...
                "min_execution_time": (
                    time_stats.get("min", 0) if isinstance(time_stats, dict) else 0
                ),
                "max_execution_time": (
                    time_stats.get("max", 0) if isinstance(time_stats, dict) else 0
                ),
                # 效率指标
                "efficiency_score": (
                    mean_best / mean_time
                ...
                "robustness_score": (
                    1 / (1 + std_best)
                    if isinstance(std_best, (int, float)) and std_best > 0
                    else 1
                ),
                # 原始数据
        ...
        if ANALYSIS_LIBS_AVAILABLE:
            try:
                ...
                # 排序（按平均最佳值）
                if (
                    hasattr(self.comparison_summary, "empty")
                    and not self.comparison_summary.empty
                ):
                    self.comparison_summary = self.comparison_summary.sort_values(
                        "mean_best_value"
                    )
            except Exception as e:
    ...
    def _save_summary(self) -> None:
        ...
        if isinstance(self.comparison_summary, list):
            ...
            try:
                if self.comparison_summary is not None and hasattr(
                    self.comparison_summary, "to_dict"
                ):
        ...
        with open(
            self.results_dir / "comparison_summary.json", "w", encoding="utf-8"
        ) as f:
        ...
        metadata = {
            ...
            "statistics": {
                ...
                "successful_optimizations": sum(
                    len([r for r in runs if not r.get("failed", False)])
    ...
    def _save_text_report(self) -> None:
        ...
        try:
            with open(report_file, "w", encoding="utf-8") as f:
                ...
                f.write(
                    f"  总执行时间: {format_execution_time(self.comparison_metadata.get('total_execution_time', 0))}\n\n"
                )
                ...
                if summary_data:
                    ...
                    for entry in summary_data:
                        ...
                        f.write(f"\n{optimizer}:\n")
                        f.write(
                            f"  成功运行: {entry.get('successful_runs', 0)}/{entry.get('successful_runs', 0) + entry.get('failed_runs', 0)}\n"
                        )
                        f.write(
                            f"  平均最佳值: {entry.get('mean_best_value', 0):.6f} ± {entry.get('std_best_value', 0):.6f}\n"
                        )
                        f.write(
                            f"  最佳值范围: [{entry.get('min_best_value', 0):.6f}, {entry.get('max_best_value', 0):.6f}]\n"
                        )
                        f.write(
                            f"  平均执行时间: {format_execution_time(entry.get('mean_execution_time', 0))}\n"
                        )
                        f.write(f"  效率分数: {entry.get('efficiency_score', 0):.6f}\n")
                        f.write(
                            f"  鲁棒性分数: {entry.get('robustness_score', 0):.6f}\n"
                        )
                ...
                for optimizer, runs in self.results.items():
                    ...
                    for i, run in enumerate(runs):
                        ...
                        if run.get("failed", False):
                            ...
                            f.write(
                                f"    执行时间: {format_execution_time(run.get('execution_time', 0))}\n"
                            )
            ...
            try:
                with pd.ExcelWriter(export_file) as writer:
                    ...
                    if hasattr(self.comparison_summary, "to_excel"):
                        self.comparison_summary.to_excel(
                    ...
                    for optimizer, runs in self.results.items():
                        for run in runs:
                            if not run.get("failed", False):
                                ...
                                # 添加最佳参数
                                if "best_params" in run:
                                    for param_name, param_value in run[
                                        "best_params"
                                    ].items():
        ...
        if export_format.lower() == "csv":
            ...
            if hasattr(self.comparison_summary, "to_csv"):
                ...
                with open(export_file, "w", newline="", encoding="utf-8") as f:
                    if (
                        isinstance(self.comparison_summary, list)
                        and self.comparison_summary
                    ):
                        writer = csv.DictWriter(
                            f, fieldnames=self.comparison_summary[0].keys()
                        )
                        writer.writeheader()
...
def compare_optimizers_legacy(*args, **kwargs):
    """向后兼容的比较函数"""
    warnings.warn(
        "compare_optimizers_legacy已弃用，请使用compare_optimizers",
        DeprecationWarning,
        stacklevel=2,
    )
    ...
    try:
        ...
        if "summary" in results and results["summary"]:
            for entry in results["summary"]:
                ...
                print(
                    f"{optimizer:12}: {mean_val:.6f} ± {std_val:.6f} ({format_execution_time(exec_time)})"
                )
