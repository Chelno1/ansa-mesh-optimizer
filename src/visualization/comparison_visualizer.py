...
class ComparisonVisualizer:
    ...
    def _plot_performance_comparison(
        ...
        ax1.bar(
            x_pos, mean_values, yerr=std_values, capsize=5, alpha=0.8, color=colors
        )
        ax1.set_xlabel("优化器")
        ax1.set_ylabel("目标值")
        ...
        for i, (mean_val, std_val) in enumerate(zip(mean_values, std_values)):
            ax1.text(
                i,
                mean_val + std_val + max(mean_values) * 0.01,
                f"{mean_val:.3f}",
                ha="center",
                va="bottom",
                fontsize=9,
        ...
        for i, optimizer in enumerate(optimizer_names):
            successful_runs = [
                r for r in results[optimizer] if not r.get("failed", False)
            ]
            values = [r["best_value"] for r in successful_runs]
            x_scatter = [i + np.random.uniform(-0.2, 0.2) for _ in values]  # 添加抖动
            ax2.scatter(
                x_scatter, values, alpha=0.6, s=50, label=optimizer, color=colors[i]
            )
    ...
    def _plot_execution_time_comparison(
        self, execution_times: Dict[str, List[float]], optimizers: List[str]
    ) -> None:
        """绘制执行时间比较图"""
        ...
        ax.bar(
            x_pos, mean_times, yerr=std_times, capsize=5, alpha=0.8, color=colors
        )
        ...
        for i, (mean_time, std_time) in enumerate(zip(mean_times, std_times)):
            ax.text(
                i,
                mean_time + std_time + max(mean_times) * 0.02,
                f"{mean_time:.1f}s",
                ha="center",
                va="bottom",
                fontsize=9,
        ...
        plt.tight_layout()
        plt.savefig(
            self.results_dir / "execution_time_comparison.png",
            dpi=300,
            bbox_inches="tight",
        )
        safe_close()
    ...
    def _plot_scatter_matrix(
        ...
        try:
            ...
            for optimizer in optimizers:
                ...
                if successful_runs:
                    ...
                    summary_entry = {
                        ...
                        "efficiency_score": (
                            np.mean(values) / np.mean(times)
                            if np.mean(times) > 0
                            else float("inf")
                        ),
                        "robustness_score": (
                            1 / (1 + np.std(values)) if np.std(values) > 0 else 1
                        ),
                    }
            ...
            available_cols = [
                col for col in numeric_cols if col in comparison_summary.columns
            ]
            ...
            fig, axes = plt.subplots(
                len(available_cols), len(available_cols), figsize=(12, 12)
            )
            ...
            plt.tight_layout()
            plt.savefig(
                self.results_dir / "scatter_matrix.png", dpi=300, bbox_inches="tight"
            )
            safe_close()
    ...
    def _plot_convergence_analysis(
        ...
        for optimizer in optimizers:
            ...
            if successful_runs:
                ...
                # 假设所有运行都有相同的迭代次数
                n_calls = (
                    successful_runs[0].get("n_calls", 30) if successful_runs else 30
                )
                iterations = [n_calls] * len(values)

                # 添加一些随机噪声来显示分布
                iterations_jittered = [
                    it + np.random.normal(0, n_calls * 0.01) for it in iterations
                ]

                ax.scatter(
                    iterations_jittered, values, alpha=0.6, label=optimizer, s=50
                )
