...
class OptimizationVisualizer:
    ...
    def __init__(self, report_dir: Optional[Path] = None):
        ...
        logger.info(f"初始化可视化器，输出目录: {self.report_dir}")

    def generate_optimization_plots(
        self, result, optimization_history=None, early_stopping=None
    ) -> None:
        """
        ...
        try:
            ...
            if hasattr(result, "to_dict"):
                ...
                if optimization_history is None and hasattr(
            ...
            logger.info(
                f"检查skopt_result: hasattr={hasattr(result, 'skopt_result')}, "
                f"skopt_result={getattr(result, 'skopt_result', None) is not None}"
            )

            if hasattr(result, "skopt_result") and result.skopt_result is not None:
                try:
                    logger.info("开始生成收敛图...")
                    ...
                    logger.debug(f"收敛图生成详细错误: {traceback.format_exc()}")
            else:
                logger.warning("跳过收敛图生成：skopt_result不存在或为None")

            # 参数重要性图（如果有skopt_result且数据足够）
            if hasattr(result, "skopt_result") and result.skopt_result is not None:
                n_calls = getattr(
                    result, "n_calls", getattr(result, "n_evaluations", 0)
                )
                if n_calls == 0 and hasattr(result.skopt_result, "func_vals"):
                    n_calls = len(result.skopt_result.func_vals)

                logger.info(f"检查参数重要性图条件: n_calls={n_calls}")

                if n_calls >= 10:  # 降低阈值，10次评估就足够生成参数重要性图
                    try:
                        ...
                        logger.debug(
                            f"参数重要性图生成详细错误: {traceback.format_exc()}"
                        )
                else:
                    logger.info(
                        f"数据不足以生成参数重要性图 (需要>=10次评估，当前有{n_calls}次)"
                    )
            ...
            if early_stopping and hasattr(early_stopping, "plot_history"):
                try:
                    early_stopping.plot_history(
                        str(self.report_dir / "early_stopping_history.png")
                    )
                    plots_generated += 1
                except Exception as e:
            ...
            if plots_generated > 0:
                logger.info(
                    f"已生成 {plots_generated} 个优化图表，保存到: {self.report_dir}"
                )
            else:
    ...
    def _plot_convergence(self, result) -> None:
        """绘制收敛图"""
        if not VISUALIZATION_AVAILABLE:
            logger.warning("可视化库不可用，跳过收敛图生成")
            return
        ...
        try:
            ...
            if skopt_result is None:
                logger.warning("没有skopt_result，跳过收敛图生成")
                return

            # logger.info(f"skopt_result类型: {type(skopt_result)}")
            # logger.info(f"skopt_result属性: {dir(skopt_result)}")
            ...
            try:
                ...
                if hasattr(skopt_result, "func_vals") and hasattr(
                    skopt_result, "x_iters"
                ):

                    # logger.info(f"func_vals长度: {len(skopt_result.func_vals)}")
                    # logger.info(f"x_iters长度: {len(skopt_result.x_iters)}")
                    ...
                    logger.info(f"收敛图已保存: {convergence_path}")

                    safe_close()
                else:
                    logger.warning("skopt_result缺少必要的属性(func_vals, x_iters)")
                    self._plot_custom_convergence(result)

            except ImportError:
                logger.warning("scikit-optimize不可用，尝试生成自定义收敛图")
                self._plot_custom_convergence(result)
    ...
    def _plot_custom_convergence(self, result) -> None:
        ...
        try:
            ...
            if skopt_result and hasattr(skopt_result, "func_vals"):
                ...
                # 如果没有skopt_result，尝试从optimization_history获取数据
                optimization_history = getattr(result, "optimization_history", [])
                if optimization_history:
                    func_vals = [
                        entry.get("result", float("inf"))
            ...
            # 绘制收敛曲线
            plt.plot(iterations, best_so_far, "r-", linewidth=2, label="最佳值")
            plt.scatter(iterations, func_vals, alpha=0.6, s=30, label="函数评估")

            plt.xlabel("迭代次数")
            plt.ylabel("目标函数值")
            plt.title(f"收敛图 - {optimizer_name}")
            plt.legend()
            ...
            convergence_path = self.report_dir / "convergence.png"
            plt.savefig(convergence_path, dpi=300, bbox_inches="tight")
            logger.info(f"自定义收敛图已保存: {convergence_path}")
    ...
    def _plot_parameter_importance(self, result) -> None:
        """绘制参数重要性图"""
        if not VISUALIZATION_AVAILABLE:
            logger.warning("可视化库不可用，跳过参数重要性图生成")
            return
        ...
        try:
            ...
            if skopt_result is not None:
                logger.info(
                    f"开始生成参数重要性图，skopt_result类型: {type(skopt_result)}"
                )
                ...
                try:
                    ...
                    if (
                        hasattr(skopt_result, "space")
                        and skopt_result.space is not None
                        and hasattr(skopt_result.space, "n_dims")
                        and skopt_result.space.n_dims > 0
                    ):
            ...
            if generation_stats is not None and len(generation_stats) > 0:
                logger.info(
                    f"使用generation_stats生成自定义参数重要性图，数据长度: {len(generation_stats)}"
                )
            ...
            # 如果都没有，记录警告
            logger.warning(
                "没有可用的数据源（skopt_result或generation_stats），跳过参数重要性图生成"
            )
    ...
    def _plot_custom_parameter_importance_from_genetic(self, result) -> None:
        ...
        try:
            ...
            logger.info(
                f"遗传算法参数重要性分析 - generation_stats: {len(generation_stats) if generation_stats else 0}, "
            ...
            if not generation_stats and not optimization_history:
                logger.warning(
                    "缺少generation_stats和optimization_history，无法生成参数重要性图"
                )
                return
            ...
            param_importance = {}
            param_names = (
                parameter_names if parameter_names else list(best_params.keys())
            )
            ...
            if optimization_history:
                ...
                logger.info(
                    f"从optimization_history提取参数数据，历史长度: {len(optimization_history)}"
                )
                ...
                for entry in optimization_history:
                    if isinstance(entry, dict):
                        ...
                        elif isinstance(params_data, list) and len(params_data) == len(
                            param_names
                        ):
                            # 参数以列表形式存储
                ...
                for name, values in param_values.items():
                    if len(values) > 1:
                        ...
                        logger.debug(
                            f"参数 {name}: {len(values)} 个值, 方差 = {param_importance[name]:.6f}"
                        )
                    else:
            ...
            if not any(param_importance.values()):
                ...
                # 基于参数值的相对大小
                max_val = (
                    max(abs(v) for v in best_params.values())
                    if best_params.values()
                    else 1.0
                )
                for name, value in best_params.items():
                    if isinstance(value, (int, float)):
                        param_importance[name] = (
                            abs(value) / max_val if max_val > 0 else 0.0
                        )
                    else:
                        param_importance[name] = 0.0
            ...
            logger.info(
                f"参数重要性排序: {[(name, val) for name, val in sorted_params]}"
            )

            # 创建条形图
            bars = plt.bar(
                range(len(param_names_sorted)),
                importance_values,
                color="skyblue",
                alpha=0.7,
                edgecolor="navy",
            ...
            for i, (bar, value) in enumerate(zip(bars, importance_values)):
                plt.text(
                    ...
                    bar.get_height() + max_importance * 0.01,
                    f"{value:.4f}",
                    ha="center",
                    va="bottom",
                    fontsize=10,
            ...
            # 添加说明文本
            final_fitness = (
                generation_stats[-1].get("best_fitness", "N/A")
                if generation_stats
                else "N/A"
            )
            plt.figtext(
    ...
    def _plot_optimization_history(
        ...
        try:
            ...
            if improvements:
                plt.scatter(
                    improvements,
                    [best_so_far[i] for i in improvements],
                    c="red",
                    s=50,
                    alpha=0.7,
                    label="Improvements",
            ...
            plt.tight_layout()
            plt.savefig(
                self.report_dir / "optimization_history.png",
                dpi=300,
                bbox_inches="tight",
            )
            safe_close()
    ...
    def plot_optimization_history(self, data, save_path=None):
        ...
        try:
            ...
            if hasattr(data, "generation_stats") and data.generation_stats:
                ...
                generations = [
                    stat.get("generation", i) for i, stat in enumerate(history)
                ]
            ...
            if len(scores) > 1:
                ...
                if score_range > 0:
                    plt.ylim(
                        min(scores) - score_range * 0.1, max(scores) + score_range * 0.1
                    )
            ...
            if save_path:
                ...
                logger.info(
                    f"优化历史图表已保存至: {self.report_dir / 'optimization_history.png'}"
                )
        ...
        try:
            ...
            if hasattr(data, "generation_stats") and data.generation_stats:
                ...
                if hasattr(data, "parameter_names"):
                    ...
                    # 如果有最佳参数，至少显示最终值
                    if hasattr(data, "best_params") and isinstance(
                ...
                logger.info(
                    f"从optimization_history提取参数演化数据，数据长度: {len(data.optimization_history)}"
                )
                ...
                for entry in data.optimization_history:
                    if isinstance(entry, dict):
                        ...
                        elif isinstance(params_entry, list) and hasattr(
                            data, "parameter_names"
                        ):
                            # 参数以列表形式存储
                            param_names = data.parameter_names
                            for i, value in enumerate(params_entry):
                                if i < len(param_names) and isinstance(
                                    value, (int, float)
                                ):
            ...
            for i, (param, values) in enumerate(params_data.items()):
                ...
                axes[i].plot(
                    generations, values, "g-", linewidth=2, marker="o", markersize=4
                )
            ...
            if save_path:
                ...
                plt.savefig(
                    self.report_dir / "parameter_evolution.png",
                    dpi=300,
                    bbox_inches="tight",
                )
                logger.info(
                    f"参数演化图表已保存至: {self.report_dir / 'parameter_evolution.png'}"
                )
    ...
    def plot_parameter_distribution(self, data, save_path=None):
        ...
        try:
            ...
            if hasattr(data, "generation_stats") and data.generation_stats:
                logger.info(
                    f"从generation_stats提取参数数据，数据长度: {len(data.generation_stats)}"
                )
                ...
                if (
                    not all_params
                    and hasattr(data, "optimization_history")
                    and data.optimization_history
                ):
                    logger.info(
                        f"从optimization_history提取参数数据，数据长度: {len(data.optimization_history)}"
                    )

                    for entry in data.optimization_history:
                        if isinstance(entry, dict):
                            # 检查不同的参数存储格式
                            params_data = entry.get(
                            ...
                            for param_name, value in params_data.items():
                                if isinstance(value, (int, float)):
                                    if param_name not in all_params:
                                        all_params[param_name] = []
                                    all_params[param_name].append(float(value))
                        elif isinstance(params_data, list) and hasattr(
                            data, "parameter_names"
                        ):
                            # 参数以列表形式存储
                            param_names = data.parameter_names
            ...
            # 方法3: 如果有skopt_result，从中提取参数
                ...
                if hasattr(skopt_result, "x_iters") and hasattr(
                    data, "parameter_names"
                ):
                    param_names = data.parameter_names
            ...
            logger.info(
                f"成功提取参数数据: {list(all_params.keys())}, 数据量: {[len(v) for v in all_params.values()]}"
            )
            ...
            for i, (param, values) in enumerate(all_params.items()):
                if values:
                    ...
                    if len(unique_values) == 1:
                        # 单一值，显示为垂直线
                        axes[i].axvline(
                            unique_values[0], color="blue", linewidth=3, alpha=0.7
                        )
            ...
            if save_path:
                ...
                plt.savefig(
                    self.report_dir / "parameter_distribution.png",
                    dpi=300,
                    bbox_inches="tight",
                )
                logger.info(
                    f"参数分布图已保存至: {self.report_dir / 'parameter_distribution.png'}"
                )
    ...
    def generate_complete_visualization_report(self, data, output_dir=None):
        ...
        try:
            ...
            try:
                if (
                    not isinstance(data, dict) and hasattr(data, "generation_stats")
                ) or (isinstance(data, dict) and "history" in data):
                    # 转换数据格式以兼容现有方法
                    if (
                        not isinstance(data, dict)
                        and hasattr(data, "generation_stats")
                        and data.generation_stats
                    ):
            ...
            logger.info(
                f"可视化报告生成完成，共生成 {plots_generated} 个图表，保存至: {output_dir}"
            )
