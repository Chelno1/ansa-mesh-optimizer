...
from src.utils.display_config import DisplayConfig, display_config, set_no_display_mode, is_no_display_mode
...
class TestDisplayConfig:
    ...
    def test_display_config_init(self):
        ...
        assert config.no_display_mode == False
        
        # 测试指定无头模式
        config = DisplayConfig(no_display=True)
        assert config.no_display_mode == True
...
class TestBackwardCompatibility:
    ...
    def test_set_no_display_mode_compatibility(self):
        ...
        assert is_no_display_mode() == True
        
        set_no_display_mode(False)
        assert is_no_display_mode() == False
    ...
    def test_safe_functions_compatibility(self):
        ...
        from src.utils.display_config import safe_show, safe_close
...
class TestThreadSafety:
    ...
    def test_thread_local_storage(self):
        """测试线程本地存储"""
        from src.utils.display_config import _thread_local, _get_current_no_display_mode
        
        # 默认情况下应该是False
        assert _get_current_no_display_mode() == False
        
        # 设置线程本地状态
        _thread_local.no_display = True
        assert _get_current_no_display_mode() == True
    ...
    def test_should_use_headless_mode_no_display(self):
        ...
        with patch.dict(os.environ, {}, clear=True):  # 清除所有环境变量
            with patch('src.utils.display_config.os.name', 'posix'):
                assert _should_use_headless_mode() == True
