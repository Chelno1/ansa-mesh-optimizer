...
class TestEvaluatorUtils(unittest.TestCase):
    ...
    def test_create_timestamped_temp_dir(self):
        """测试创建时间戳临时目录"""
        with patch('src.evaluators.utils.os.getcwd', return_value='/test'):
            with patch('src.evaluators.utils.os.makedirs') as mock_makedirs:
    ...
    def test_create_timestamped_temp_dir_failure(self):
        """测试临时目录创建失败的情况"""
        with patch('src.evaluators.utils.os.getcwd', return_value='/test'):
            with patch('src.evaluators.utils.os.makedirs', side_effect=OSError("Permission denied")):
                temp_dir = create_timestamped_temp_dir()
                # 应该回退到当前目录
    ...
    def test_create_temp_config(self):
        ...
        with patch('src.evaluators.utils.tempfile.NamedTemporaryFile') as mock_temp:
            mock_file = MagicMock()
