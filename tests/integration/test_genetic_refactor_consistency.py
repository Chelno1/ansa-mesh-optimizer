...
class GeneticRefactorConsistencyTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.param_space = TestParamSpace(self.bounds, self.param_types, self.param_names)
    ...
    def test_api_consistency(self):
        ...
        optimizer_refactored = RefactoredGeneticOptimizer(
            self.param_space, self.evaluator_refactored, self.config, self.genetic_config
        )
    ...
    def test_optimization_results_similarity(self):
        ...
        # 验证结果属性存在
        self.assertTrue(hasattr(result_original, 'best_params') or 'best_params' in result_original)
        self.assertTrue(hasattr(result_refactored, 'best_params'))
        self.assertTrue(hasattr(result_original, 'best_score') or 'best_value' in result_original)
        self.assertTrue(hasattr(result_refactored, 'best_score'))
        
        # 获取最佳值进行比较
        original_best = result_original.best_score if hasattr(result_original, 'best_score') else result_original.get('best_value', float('inf'))
        refactored_best = result_refactored.best_score
        
        # 确保值为数值类型
        original_best = float(original_best) if original_best is not None else float('inf')
        refactored_best = float(refactored_best) if refactored_best is not None else float('inf')
    ...
    def test_configuration_consistency(self):
        ...
        # 验证配置参数
        self.assertEqual(optimizer_original.genetic_config.population_size, 
                        optimizer_refactored.genetic_config.population_size)
        self.assertEqual(optimizer_original.genetic_config.max_generations, 
                        optimizer_refactored.genetic_config.max_generations)
        self.assertEqual(optimizer_original.genetic_config.mutation_rate, 
                        optimizer_refactored.genetic_config.mutation_rate)
        self.assertEqual(optimizer_original.genetic_config.crossover_rate, 
    ...
    def test_export_functionality(self):
        ...
        # 运行小规模优化
        result = optimizer_refactored.optimize(n_calls=50)
    ...
    def test_modular_components(self):
        ...
        # 测试进化引擎
        evolution_engine = EvolutionEngine(self.bounds, self.param_types, self.genetic_config)
    ...
    def test_diversity_metrics(self):
        ...
        # 运行优化
        result = optimizer_refactored.optimize(n_calls=60)
